...
public class KniffelClient : MonoBehaviour
{
    ...
    private void UpdatePunkteliste(string data)
    {
        foreach (string player in data.Replace("[#]","|").Split('|'))
        {
            ...
            p.KleineStraße.SetPoints(Int32.Parse(infos[16].Split('~')[1]), p.PlayerColor);
            p.KleineStraße.used = bool.Parse(infos[16].Split('~')[0]);
            p.GroßeStraße.SetPoints(Int32.Parse(infos[17].Split('~')[1]), p.PlayerColor);
            p.GroßeStraße.used = bool.Parse(infos[17].Split('~')[0]);
    ...
    IEnumerator StartTurnDelayed(int id)
    {
        ...
        // wenn Client dran ist, dann würfel aktivieren
    ...
    IEnumerator StartWuerfelAnimation()
    {
        ...
        for (int i = 1; i <= 5; i++)
        {
            ...
            safes[i - 1].GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/GUI/würfel");
        }

        for (int i = 0; i < board.GetPlayerTurn().safewuerfel.Count; i++)
            safes[i].GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/GUI/würfel " + board.GetPlayerTurn().safewuerfel[i]);
        ...
        for (int i = 0; i < board.GetPlayerTurn().unsafewuerfe.Count; i++)
        {
            ...
            unsafes[randomwuerfel].GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/GUI/würfel " + board.GetPlayerTurn().unsafewuerfe[i]);
    ...
    private void AktualisiereKategorien()
    {
        ...
        if (!p.KleineStraße.used)
        {
            ...
            zahlen.Contains(3) && zahlen.Contains(4) && zahlen.Contains(5) && zahlen.Contains(6))
                p.KleineStraße.DisplayTest(30);
            else
                p.KleineStraße.DisplayTest(0);
            if (board.GetPlayerTurn().name == Config.PLAYER_NAME)
                p.KleineStraße.button.GetComponent<Button>().interactable = true;
        }
        if (!p.GroßeStraße.used)
        {
            ...
            zahlen.Contains(2) && zahlen.Contains(3) && zahlen.Contains(4) && zahlen.Contains(5) && zahlen.Contains(6))
                p.GroßeStraße.DisplayTest(40);
            else
                p.GroßeStraße.DisplayTest(0);
            if (board.GetPlayerTurn().name == Config.PLAYER_NAME)
                p.GroßeStraße.button.GetComponent<Button>().interactable = true;
        }
        if (!p.Kniffel.used)
        {
    ...
    private void SafeUnsafeWuerfel(string data)
    {
        ...
        if (type == "Safe")
        {
            for (int i = 0; i < WuerfelBoard.transform.GetChild(0).GetChild(0).childCount; i++)
            {
                ...
                if (wuerfel.sprite.name.EndsWith("" + zahl) && wuerfel.gameObject.activeInHierarchy)
                {
                    ...
                    wuerfel.sprite = Resources.Load<Sprite>("Images/GUI/würfel");

                    for (int j = 0; j < SafeWuerfel.Count; j++)
                    {
                        if (SafeWuerfel[j].sprite.name.Equals("würfel"))
                        {
                            SafeWuerfel[j].sprite = Resources.Load<Sprite>("Images/GUI/würfel " + zahl);
                            break;
                        }
                    }
                    break;
        ...
        else if (type == "Unsafe")
        {
            ...
            {
                if (SafeWuerfel[i].sprite.name.Replace("würfel ", "").Equals("" + zahl))
                {
                    board.GetPlayerTurn().safewuerfel.Remove(zahl);
                    board.GetPlayerTurn().unsafewuerfe.Add(zahl);
                    SafeWuerfel[i].sprite = Resources.Load<Sprite>("Images/GUI/würfel");
                    ...
                    for (int j = 0; j < WuerfelBoard.transform.GetChild(0).GetChild(0).childCount; j++)
                    {
                        ...
                        {
                            boardImage.sprite = Resources.Load<Sprite>("Images/GUI/würfel " + zahl);
    ...
    public void ClientSafeUnsafeWuerfel(GameObject wuerfel)
    {
        ...
        if (board.GetPlayerTurn().name != Config.PLAYER_NAME)
            return;
        string spritename = wuerfel.GetComponent<Image>().sprite.name.Replace("würfel", "");
        if (spritename.Length == 0)
            return;
        int zahl = Int32.Parse(wuerfel.GetComponent<Image>().sprite.name.Replace("würfel ", ""));
        string type = "";
        ...
        if (type == "Safe")
        {
            for (int i = 0; i < WuerfelBoard.transform.GetChild(0).GetChild(0).childCount; i++)
            {
                ...
                if (wuerfelImage.sprite.name.EndsWith("" + zahl) && wuerfelImage.gameObject.activeInHierarchy)
                {
                    ...
                    wuerfelImage.sprite = Resources.Load<Sprite>("Images/GUI/würfel");
    ...
    {
        // TODO falsche spalte wird eingeblendet mit vorschau (nur wenn zuschnell gewählt wird)
        ...
        switch (kategorie.name)
        {
            ...
            case "KleineStraße":
                if (zahlen.Contains(1) && zahlen.Contains(2) && zahlen.Contains(3) && zahlen.Contains(4) ||
                    zahlen.Contains(2) && zahlen.Contains(3) && zahlen.Contains(4) && zahlen.Contains(5) ||
                    zahlen.Contains(3) && zahlen.Contains(4) && zahlen.Contains(5) && zahlen.Contains(6))
                    player.KleineStraße.SetPoints(30, player.PlayerColor);
                else
                    player.KleineStraße.SetPoints(0, player.PlayerColor);
                break;
            case "GroßeStraße":
                if (zahlen.Contains(1) && zahlen.Contains(2) && zahlen.Contains(3) && zahlen.Contains(4) && zahlen.Contains(5) ||
                    zahlen.Contains(2) && zahlen.Contains(3) && zahlen.Contains(4) && zahlen.Contains(5) && zahlen.Contains(6))
                    player.GroßeStraße.SetPoints(40, player.PlayerColor);
                else
                    player.GroßeStraße.SetPoints(0, player.PlayerColor);
                break;
            case "Kniffel":
                if ((zahlen[0] == zahlen[1]) && (zahlen[0] == zahlen[2]) && (zahlen[0] == zahlen[3]) && (zahlen[0] == zahlen[4]))
    ...
    private void AktualisierePointsKategorien(KniffelPlayer p)
    {
        ...
        if (!p.KleineStraße.used)
            p.KleineStraße.button.GetComponentInChildren<TMP_Text>().text = "";
        if (!p.GroßeStraße.used)
            p.GroßeStraße.button.GetComponentInChildren<TMP_Text>().text = "";
        if (!p.Kniffel.used)
        ...
        p.SummeUntererTeil.SetPoints(p.Dreierpasch.getDisplay() + p.Viererpasch.getDisplay() + p.FullHouse.getDisplay() + p.KleineStraße.getDisplay() + p.GroßeStraße.getDisplay() + p.Kniffel.getDisplay() + p.Chance.getDisplay(), p.PlayerColor);
