var LibraryFileUploadManager = {
    ...
    $Unity_FileUploadManager: {
                    ...
                    for (var yy = Math.floor(j * ratio_h); yy < (j + 1) * ratio_h; yy++) {
                        ...
                        for (var xx = Math.floor(i * ratio_w); xx < (i + 1) * ratio_w; xx++) {
                            ...
                            if (w >= -1 && w <= 1) {
                                ...
                                if (weight > 0) {
                                    var dx4 = 4 * (xx + yy * W);
                                    gx_r += weight * data[dx4];
                                    gx_g += weight * data[dx4 + 1];
                                    gx_b += weight * data[dx4 + 2];
                                    gx_a += weight * data[dx4 + 3];
                                    weights += weight;
                                }
                            }
                        }
                ...
                while (1) {
                    ...
                    if (img.charCodeAt(head) == 255 & img.charCodeAt(head + 1) == 216) {
                        ...
                        if (img.charCodeAt(head) == 255 & img.charCodeAt(head + 1) == 225) {
                            ...
                            var count = bigEndian ? segment.charCodeAt(18) * 256 + segment.charCodeAt(19) : segment.charCodeAt(18) + segment.charCodeAt(19) * 256;
        ...
        checkExistAPI: function() {
            return !!(window.File && window.FileReader && window.Promise);
        },
        ...
        createRequiredHTML: function() {
            ...
            if (fileUploaderElem == null) {
                ...
                fileUploaderElem = document.createElement('div');
            ...
            if (fileListElem == null) {
                fileListElem = document.createElement('div');
        ...
        handleFileSelect: function(evt) {
            ...
            for (var i = 0, f; f = files[i]; i++) {
                results.push({
                    ...
                    "filePath": "",
                    "isSuccess": false,
                    "errorCode": Unity_FileUploadManager.ERROR_CODE.NONE
                });
            }
    ...
    Unity_FileUploadManager_SetDescription: function(descriptionStr) {
        ...
        descriptionStr = Pointer_stringify(descriptionStr);
        if (Unity_FileUploadManager.isDropInput) {
    ...
    Unity_FileUploadManager_SetAllowedFileName: function(filenameRegStr) {
        ...
        filenameRegStr = Pointer_stringify(filenameRegStr);
